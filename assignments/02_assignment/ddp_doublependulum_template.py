...
class DDPSolverLinearDyn(DDPSolver):
    ...
    def cost_running(self, i, x, u):                                    
        ...
        cost = 0.5*np.dot(x, np.dot(self.H_xx[i,:,:], x)) \
                ...
                + 0.5*self.lmbda*np.dot(u.T, u) + ...                           # Add term penalizing the torque provided by the second motor with weight self.underact  
        return cost
    ...
    def cost_running_u(self, i, x, u):
        ...
        c_u = self.lmbda * u + ...                                              # Modify accordingly the gradient of the running cost w.r.t. u                             
        return c_u
    ...
    def cost_running_uu(self, i, x, u):
        ...
        return self.lmbda * np.eye(self.nu) + ...                               # Modify accordingly the hessian of the running cost w.r.t. u
...
class DDPSolverDoublePendulum(DDPSolverLinearDyn):
    ...
    def f(self, x, u):
        ...
        if SELECTION_MATRIX == 1:
            self.S = ...                                   # Selection matrix
            u = ...                                        # When using the selection matrix method, the correct dynamics must be used in the forward pass of DDP.
    ...
    def f_x(self, x, u):
        ...
        if SELECTION_MATRIX==1 and ACTUATION_PENALTY==0:
            self.Fu[nv:, :] = ...                                      # Partial derivatives of system dynamics w.r.t. u    
    ...
    def start_simu(self, X, U, KK, dt_sim):
        ...
        for i in range(0, N_sim):
            ...
            q,v,dv,f = simu.simulate(tau, dt_sim)
